...
import { useGameAI } from '../context/GameAIContext'; // חיבור ל־Context
...
const AIChatBox = () => {
  ...
  // שימוש ב־Context כדי לקבל את הנתונים של המשחק
  const gameData = useGameAI() || {};
  const { players, opponents, gameTime, isRunning } = gameData;
  ...
  const sendMessage = async () => {
    ...
    // יצירת הודעת הקשר עם כל נתוני המשחק
    const contextMessage = `
You are an assistant helping a basketball coach during a live game.
Here is the real-time game data:
...
`;
    ...
    try {
      const reply = await fetchOpenAIResponse(contextMessage);
